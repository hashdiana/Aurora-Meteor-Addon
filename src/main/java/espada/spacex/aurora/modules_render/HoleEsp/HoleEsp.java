package espada.spacex.aurora.modules_render.HoleEsp;
...
public class HoleEsp extends Modules {
    public HoleEsp() {
        super(Aurora.AURORA, "HoleEsp", "Displays holes that you will take less damage in.");
    }
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        if (!holes.isEmpty()) {
            holes.sort(Comparator.comparingDouble(hole -> distance(mc.player.getPos(), Vec3d.ofCenter(hole.pos1))));
                ...
                if (hole.isDouble()) {
                    if (ignoreOwn.get() && (mc.player.getBlockPos().equals(hole.pos1) || mc.player.getBlockPos().equals(hole.pos2))
                        ...
                        if (canRender(hole.pos1)
                            && distanceXZ(mc.player.getPos(), Vec3d.ofCenter(hole.pos1)) <= horizontalDistance.get()
                            && distanceY(mc.player.getY(), hole.pos1.getY() + 0.5) <= verticalDistance.get()) {
                        ...
                        if (canRender(hole.pos2)
                            && distanceXZ(mc.player.getPos(), Vec3d.ofCenter(hole.pos2)) <= horizontalDistance.get()
                            && distanceY(mc.player.getY(), hole.pos2.getY() + 0.5) <= verticalDistance.get()) {
    ...
    public class RenderBlock {
        ...
        public void tick() {
            if (distanceXZ(mc.player.getPos(), Vec3d.ofCenter(pos)) > horizontalDistance.get()
                || distanceY(mc.player.getY(), pos.getY() + 0.5) > verticalDistance.get()
        ...
        public boolean isInvalid() {
            return (!fadeOut.get()
                && (distanceXZ(mc.player.getPos(), Vec3d.ofCenter(pos)) > horizontalDistance.get()
                || distanceY(mc.player.getY(), pos.getY() + 0.5) > verticalDistance.get()))
