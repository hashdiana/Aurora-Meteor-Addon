...
public class CevBreaker extends Modules {
    public CevBreaker() {
        super(Aurora.AURORA, "Cev Breaker", "Break crystals over a ppl's head to deal massive damage!");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (!blockState.isOf(Blocks.OBSIDIAN) && !crystalThere && (mc.player.getMainHandStack().getItem().equals(Items.OBSIDIAN) || switchDelayLeft <= 0)) {
            if (BlockUtils.canPlace(blockPos)) {
                ...
                switch (switchMode.get()) {
                    ...
                    case InvSwitch -> BOInvUtils.invSwitch(obsidian.slot());
                    case PickSilent -> BOInvUtils.pickSwitch(obsidian.slot());
                }
        ...
        if (!BOBlockUtil.solid(blockPos)) progress = 0D;
        ...
        if ((mc.player.getInventory().selectedSlot == pickSlot || switchDelayLeft <= 0) && crystalThere && blockState.isOf(Blocks.OBSIDIAN)) {
            Direction direction = BOEntityUtils.rayTraceCheck(blockPos, true);
            switch (mode.get()) {
                case Instant -> {
                    if (!startedYet) {
                ...
                case Packet -> {
                    ...
                    timer = startedYet ? timer : BOBlockUtil.getBlockBreakingSpeed(blockState, blockPos, pickSlot);
    ...
    private void onRender2D(Render2DEvent event) {
        ...
        if (NametagUtils.to2D(pos, scale.get())) {
            ...
            String progressText = mode.get().equals(Mode.Instant) ? (!BOBlockUtil.solid(blockPos) ? "Waiting" : "Mining") : String.format("%.2f", progress) + "%";
            textRenderer.render(
                progressText,
