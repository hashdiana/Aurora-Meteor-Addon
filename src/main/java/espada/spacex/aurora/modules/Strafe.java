...
public class Strafe extends Modules {
    ...
    private final Setting<Boolean> earthh4ckMode = sgGeneral.add(new BoolSetting.Builder()
        .name("3arthh4ck-Mode")
        .description("only air to boost")
        .defaultValue(true)
        .build()
    );
    ...
    private final Setting<Boolean> pBur = sgGeneral.add(new BoolSetting.Builder()
        .name("burrowbypass")
    ...
    public Strafe() {
        super(Aurora.ExtendMove, "strafe", "Increase speed and control.");
    }
    ...
    Timer timerClass = meteordevelopment.meteorclient.systems.modules.Modules.get().get(Timer.class);
    LongJump longJump = meteordevelopment.meteorclient.systems.modules.Modules.get().get(LongJump.class);
    Flight flight = meteordevelopment.meteorclient.systems.modules.Modules.get().get(Flight.class);
    ElytraFly efly = meteordevelopment.meteorclient.systems.modules.Modules.get().get(ElytraFly.class);
    ...
    private void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (earthh4ckMode.get() && (mc.player.isOnGround())) return ;
        if (BOEntityUtils.isBurrowed(mc.player, !pBur.get())) return;
        ...
        if (BOEntityUtils.isWebbed(mc.player) && webbedPause.get() == WebbedPause.Always) return;
        if (BOEntityUtils.isWebbed(mc.player) && !mc.player.isOnGround() && webbedPause.get() == WebbedPause.OnAir) return;

        if (mc.player.isOnGround()) {
            timerClass.setOverride(PlayerUtils.isMoving() ? (groundTimer.get()) : Timer.OFF);
        } else
            timerClass.setOverride(PlayerUtils.isMoving() ? (airTimer.get()) : Timer.OFF);
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        if (BOEntityUtils.doesBoxTouchBlock(mc.player.getBoundingBox(), Blocks.COBWEB) && webbedPause.get() == WebbedPause.Always) return;
        if (BOEntityUtils.doesBoxTouchBlock(mc.player.getBoundingBox(), Blocks.COBWEB) && !mc.player.isOnGround() && webbedPause.get() == WebbedPause.OnAir) return;
