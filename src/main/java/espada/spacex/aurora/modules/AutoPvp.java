...
public class AutoPvp extends Modules {
    public AutoPvp() {
        super(Aurora.EAncillary, "Auto CC", "Follows people using baritone. Best for crystalpvp.cc");
    }
    ...
    private void onRender(Render3DEvent event) {
        ...
        if (shouldSuicide) {
            if (!meteordevelopment.meteorclient.systems.modules.Modules.get().isActive(Suicide.class))
                meteordevelopment.meteorclient.systems.modules.Modules.get().get(Suicide.class).toggle();
        } else {
            if (meteordevelopment.meteorclient.systems.modules.Modules.get().isActive(Suicide.class))
                meteordevelopment.meteorclient.systems.modules.Modules.get().get(Suicide.class).toggle();
        }
        ...
        if (inRange && surround.get() && !shouldSuicide) {
            if (!meteordevelopment.meteorclient.systems.modules.Modules.get().isActive(SurroundPlus.class))
                meteordevelopment.meteorclient.systems.modules.Modules.get().get(SurroundPlus.class).toggle();
