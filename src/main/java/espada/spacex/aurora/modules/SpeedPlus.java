...
public class SpeedPlus extends Modules {
    public SpeedPlus() {
        super(Aurora.ExtendMove, "Speed+", "Speeeeeeeed.");
    }
    ...
    private final Setting<Boolean> burslow = sgPause.add(new BoolSetting.Builder()
        .name("BurrowSlow")
        .description("")
        .defaultValue(false)
        .build()
    );
    private final Setting<Double> burspeed = sgGeneral.add(new DoubleSetting.Builder()
        .name("BurrowSpeed")
        .description("")
        .defaultValue(0.01f)
        .min(0.001f)
    ...
    public void onMove(PlayerMoveEvent event) {
        ...
        if (mc.player != null && mc.world != null) {
            if (meteordevelopment.meteorclient.systems.modules.Modules.get().get(HoleSnap.class).isActive()) {
                return;
            }
            ...
            if (mc.world.getBlockState(new BlockPos(mc.player.getBlockPos())).getBlock() != Blocks.OBSIDIAN && burslow.get()) {
                velocity = Math.max(speed.get(), velocity * burspeed.get());
            }
