...
public class AutoAnvil extends Modules {
    public AutoAnvil() {
        super(Aurora.Extendcombat, "Auto Anvil", "Automatically places anvils above players to destroy helmets.");
    }
    ...
    private int timer;
    private final Timer helperTimer = new Timer();
    ...
    private void onRender3D(Render3DEvent event) {
        ...
        if (renderTargetEsp.get() && target != null && canRenderTarget) {
            RenderUtils.drawJello(event.matrices, target, color.get());
        }
        ...
        if (renderHelper.get()) {
            ...
            renderHelperPlacing.forEach(r -> {
                ...
                event.renderer.box(r.pos, RenderUtils.injectAlpha(helperSideColor.get(), (int) Math.round(helperSideColor.get().a * progress)), RenderUtils.injectAlpha(helperLineColor.get(), (int) Math.round(helperLineColor.get().a * progress)), helperShapeMode.get(), 0);
            });
        }
        ...
        if (renderAnvil.get()) {
            ...
            renderAnvilPlacing.forEach(r -> {
                ...
                event.renderer.box(r.pos, RenderUtils.injectAlpha(anvilSideColor.get(), (int) Math.round(anvilSideColor.get().a * progress)), RenderUtils.injectAlpha(anvilLineColor.get(), (int) Math.round(anvilLineColor.get().a * progress)), anvilShapeMode.get(), 0);
            });
        }
    }
    ...
    private List<BlockPos> getHelper(BlockPos block) {
        ...
        if (!BOBlockUtil.replaceable(block)) return list;
