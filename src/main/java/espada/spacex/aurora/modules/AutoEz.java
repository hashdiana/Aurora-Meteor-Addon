...
import static espada.spacex.aurora.modules.AutoEz.MessageMode.AlexJonnyMine;
import static espada.spacex.aurora.modules.AutoEz.MessageMode.Kry4tal;
...
public class AutoEz extends Modules {
    public AutoEz() {
        super(espada.spacex.aurora.Aurora.EAncillary, "Auto EZ", "Sends message after enemy dies (too EZ nn's).");
    }
    ...
    private final Setting<List<String>> killMessages = sgKill.add(new StringListSetting.Builder()
        .name("Kill Messages")
        .description("Messages to send when killing an enemy with aurora message mode on")
    ...
    private final String[] Aurora = new String[]{
        "%s Aurora User Version Kill You"
    };
    private final String[] QuickMacro = new String[]{
        "Aurora用户击杀了 %s 其他主播小心点 "
    };
    private final String[] ACE_joni = new String[]{
        "%s 这不行那不行 我乔尼哥要你有何用? "
    };
    private final String[] Xin = new String[]{
        "%s 我正在使用Aurora 游玩中国2B2T "
    };
    private final String[] Dyyibang = new String[]{
        "%s 可以详细说一下你买Future被骗200r的过程吗? "
    };
    private final String[] Test = new String[]{
        "/kill"
    };
    private final String[] KRY4TAL = new String[]{
        "LJM语音所有人里面都在说 %s 牛逼 %s 牛逼"
    };
    private final String[] AlexJonnyMine = new String[]{
        "%s 挖掘失败 "
    };
    private final String[] Xsclub = new String[]{
        "首先Dyyibang不是我在玩 其次EZ %s"
    };
    ...
    private void sendKillMessage() {
        switch (killMsgMode.get()) {
            ...
            case Aurora -> {
                int num = r.nextInt(0,Aurora.length);
                if (num == lastNum) {
                    num = num < Aurora.length - 1 ? num + 1 : 0;
                }
                lastNum = num;
                messageQueue.add(0, new Message(Aurora[num].replace("%s", name == null ? "You" : name), true));
            }
            case AlexJonnyMine -> {
                int num = r.nextInt(0,AlexJonnyMine.length);
                if (num == lastNum) {
                    num = num < AlexJonnyMine.length - 1 ? num + 1 : 0;
                }
                lastNum = num;
                messageQueue.add(0, new Message(AlexJonnyMine[num].replace("%s", name == null ? "You" : name), true));
            }
            case Xin -> {
                int num = r.nextInt(0, Xin.length);
                if (num == lastNum) {
                    num = num < Xin.length - 1 ? num + 1 : 0;
                }
                lastNum = num;
                messageQueue.add(0, new Message(Xin[num].replace("%s", name == null ? "You" : name), true));
            }
            case Test -> {
                int num = r.nextInt(0,Test.length);
                if (num == lastNum) {
                    num = num < Test.length - 1 ? num + 1 : 0;
                }
                lastNum = num;
                messageQueue.add(0, new Message(Test[num].replace("%s", name == null ? "You" : name), true));
            }
            case Kry4tal -> {
                int num = r.nextInt(0,KRY4TAL.length);
                if (num == lastNum) {
                    num = num < KRY4TAL.length - 1 ? num + 1 : 0;
                }
                lastNum = num;
                messageQueue.add(0, new Message(KRY4TAL[num].replace("%s", name == null ? "You" : name), true));
            }
            case Xsclub -> {
                int num = r.nextInt(0,Xsclub.length);
                if (num == lastNum) {
                    num = num < Xsclub.length - 1 ? num + 1 : 0;
                }
                lastNum = num;
                messageQueue.add(0, new Message(Xsclub[num].replace("%s", name == null ? "You" : name), true));
            }

            case QuickMacro -> {
                int num = r.nextInt(0,QuickMacro.length);
                if (num == lastNum) {
                    num = num < QuickMacro.length - 1 ? num + 1 : 0;
                }
                lastNum = num;
                messageQueue.add(0, new Message(QuickMacro[num].replace("%s", name == null ? "You" : name), true));
            }
            case ACE_joni -> {
                int num = r.nextInt(0,ACE_joni.length);
                if (num == lastNum) {
                    num = num < ACE_joni.length - 1 ? num + 1 : 0;
                }
                lastNum = num;
                messageQueue.add(0, new Message(ACE_joni[num].replace("%s", name == null ? "You" : name), true));
            }
            case Dyyibang -> {
                int num = r.nextInt(0,Dyyibang.length);
                if (num == lastNum) {
                    num = num < Dyyibang.length - 1 ? num + 1 : 0;
                }
                lastNum = num;
                messageQueue.add(0, new Message(Dyyibang[num].replace("%s", name == null ? "You" : name), true));
            }
            case Blackout -> {
                if (!killMessages.get().isEmpty()) {
                    ...
                    lastNum = num;
                    messageQueue.add(0, new Message(killMessages.get().get(num).replace("%s", name == null ? "You" : name), true));
                }
            }
            case NoClue -> {
