...
public class MineHud extends HudElement {
    ...
    public static final HudElementInfo<MineHud> INFO = new HudElementInfo<>(Aurora.HUD_EDIT, "MineHud", "Welcomes you.", MineHud::new);

    private void runTick(HudRenderer renderer) {
        if (needsCompile) {
            Parser.Result result = Parser.parse("Auto Mine: {meteor.is_module_active(\"AuroraMine\") ? #2 \"ON\" : #3 \"OFF\"} #1{meteor.get_module_info(\"AuroraMine\")}");
