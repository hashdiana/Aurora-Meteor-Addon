...
public class AuroraArray extends HudElement {
    ...
    private final Setting<Boolean> combatpage = sgGeneral.add(new BoolSetting.Builder()
        .name("MainPage")
        .description("Only shows aurora modules in the hud.")
        .defaultValue(false)
        .build()
    );
    ...
    public static final HudElementInfo<AuroraArray> INFO = new HudElementInfo<>(Aurora.HUD_EDIT, "AuroraArray", "An ArrayList for aurora features.", AuroraArray::new);
    ...
    private List<Module> getModules() {
        return Modules.get().getActive()
            .stream()
            .filter(module -> !combatpage.get() || module.category.equals(Aurora.Extendcombat))
            .collect(Collectors.toList());
    }
